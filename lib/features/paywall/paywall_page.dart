...
class PaywallPage extends StatelessWidget {
  const PaywallPage({
    super.key,
    this.placementType = PlacementType.personal,
    this.photo,
  });
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [
        ...
        BlocProvider(
          create: (context) =>
              getIt<PaywallCubit>(param1: placementType, param2: photo),
        ),
      ],
        ...
        child: Scaffold(
          body: BlocBuilder<PaywallCubit, PaywallState>(
            builder: (context, state) {
              ...
              //чтобы смотреть на android в дебаге
              if (kDebugMode) {
                return PaywallTypeUsual(
                  paywallState: PaywallUsualSwitchState.isdefault(),
                );
              }
